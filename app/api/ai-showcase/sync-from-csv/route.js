...
export async function POST(request) {
  ...
  const basePath = '/ai-showcase/sync-from-csv';
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      console.warn(`[ai-showcase/sync-from-csv] primary failed ${resp.status}: ${text}`);
    ...
    console.error('[ai-showcase/sync-from-csv] proxy error:', err);
    return NextResponse.json(
