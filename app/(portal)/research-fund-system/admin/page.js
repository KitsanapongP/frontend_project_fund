...
function AdminPageContent({ initialPage = 'dashboard', basePath = '/research-fund-system/admin' }) {
  ...
  const pageFromPath = useCallback(
    (path) => {
      ...
      const baseSegments = String(basePath || '/research-fund-system/admin')
        .replace(/^\//, '')
        .split('/')
        .filter(Boolean);

      const matchesBase = baseSegments.every((segment, index) => segments[index] === segment);
      if (!matchesBase) return 'dashboard';

      return normalizePage(segments[baseSegments.length] || 'dashboard');
    },
