...
export function ModalAddType({ isOpen, onClose, type, data }) {
  if (!isOpen) return null;

  const [departmentName, setdepartmentName] = useState(data.name);
  const [OldepartmentName, setolddpartmentName] = useState(data.name);
  ...
  const handleSubmit = async (e) => {
    ...
    Swal.fire({
      ...
      text: departmentName ? `ชื่อประเภท: ${departmentName}` : "",
      icon: "question",
      ...
      if (result.isConfirmed) {
        ...
        if (OldepartmentName === departmentName) {
          Swal.fire({
            ...
            text: "ไม่สามารถแก้ไขประเภทเป็นค่าเดิมได้",
            icon: "error",
        ...
        try {
          ...
          let response;
          if (type == 1) {
            response = await AddDataType(token, departmentName);
          } else {
            // console.log(data.id)
            response = await EditDatadeType(token, departmentName, data.id);
          }
        ...
        <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
              {type == 1 ? "เพิ่มประเภทใหม่" : "แก้ไขประเภท"}
            </h3>
            <button
              type="button"
              onClick={onClose}
