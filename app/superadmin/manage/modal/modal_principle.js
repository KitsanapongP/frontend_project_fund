...
export function ModalAddPrinciple({ isOpen, onClose, type, data }) {
  if (!isOpen) return null;

  const [principle, setprinciple] = useState(data.name);
  const [oldprinciple, setoldprinciple] = useState(data.name);
  ...
  const handleSubmit = async (e) => {
    ...
    Swal.fire({
      ...
      text: principle ? `ชื่อหลักธรรมภิบาล: ${principle}` : "",
      icon: "question",
      ...
      if (result.isConfirmed) {
        // console.log("Submitted Strategic Name: ", principle);

        // เพิ่มหรือส่งข้อมูลกลับ
        // เช่น: onSave(principle);
        if (oldprinciple === principle) {
          Swal.fire({
            title: "เกิดข้อผิดพลาด",
            text: "ไม่สามารถแก้ไขหลักธรรมภิบาลเป็นค่าเดิมได้",
            icon: "error",
        ...
        try {
          ...
          let response;
          if (type == 1) {
            response = await AddDataPrinciple(token, principle);
          } else {
            // console.log(data.id)
            response = await EditDatadePrinciple(token, principle, data.id);
          }
        ...
        <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
              {type == 1 ? "เพิ่มหลักธรรมภิบาลใหม่" : "แก้ไขหลักธรรมภิบาล"}
            </h3>
            <button
              type="button"
              onClick={onClose}
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div>
                ...
                <input
                  ...
                  placeholder="กรุณากรอกชื่อหลักธรรมภิบาล"
                  required
                  value={principle}
                  onChange={(e) => {
                    // onprincipleChange(e.target.value);
