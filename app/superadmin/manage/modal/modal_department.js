...
export function ModalAdddepartment({ isOpen, onClose, type, department }) {
  if (!isOpen) return null;

  const [departmentName, setdepartmentName] = useState(department.name);
  const [OldepartmentName, setolddpartmentName] = useState(department.name);
  ...
  const handleSubmit = async (e) => {
    ...
    Swal.fire({
      ...
      text: departmentName ? `ชื่อหน่วยงาน: ${departmentName}` : "",
      icon: "question",
      ...
      if (result.isConfirmed) {
        // console.log("Submitted Strategic Name: ", departmentName);

        // เพิ่มหรือส่งข้อมูลกลับ
        // เช่น: onSave(departmentName);
        if (OldepartmentName === departmentName) {
          Swal.fire({
            title: "เกิดข้อผิดพลาด",
            text: "ไม่สามารถแก้ไขหน่วยงานเป็นค่าเดิมได้",
            icon: "error",
        ...
        try {
          ...
          let response;
          if (type == 1) {
            response = await AddDatadepartment(token, departmentName);
          } else {
            console.log(department.id);
            response = await EditDatadepartment(
              token,
              departmentName,
              department.id
            );
        ...
        <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
              {type == 1 ? "เพิ่มหน่วยงานใหม่" : "แก้ไขหน่วยงาน"}
            </h3>
            <button
              type="button"
              onClick={onClose}
